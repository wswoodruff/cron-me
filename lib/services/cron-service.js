...
module.exports = class CronMeService extends Schmervice.Service {
    ...
    async runCrons() {
        ...
        // Repeating intervals from start date
        ...
        const dayScheduleRuns = await Cron.query()
            .whereRaw('enabled = ?', true)
            .where('type', 'day-schedule')
            .where('cronExpression', 'daily')
            .whereRaw(`?? <= (to_char(now(), 'HH24:MI:SS')::time + interval ${GRACE_PERIOD_INTERVAL})`, 'time')
            // Only capture the very minute this matches
            .whereRaw(`?? + interval \'1 minute\' > (to_char(now(), 'HH24:MI:SS')::time + interval ${GRACE_PERIOD_INTERVAL})`, 'time');
